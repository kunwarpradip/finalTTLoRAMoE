...
from model_with_routerloss import CustomLightningModule
from utils import get_tokenizer, load_mixed_datasets, parse_experts, get_ttlora_shape, get_ttlora_rank
from utils import load_new_model_for_sequence_classification_from_local_path, load_dataset_, load_mixed_datasets_with_expertlabels
from _TTLoRAWrapper_TensorMultiplication import AdaptCores_and_Test_Individual, TTLoRALinearWrapper_withcores
from _TTMoE_with_routerloss_multitask_wrapper import MoEsparseRouting, MoEsparseRoutingForClassification
...
def train_moe_without_ray(config):
    ...
    if "roberta" in config["model_name"]:
        model = MoEsparseRouting(model, config=config)
        model.base_module.classifier = MoEsparseRoutingForClassification(
                                base_classifier=model.base_module.classifier, 
                                config=config,)
    elif "llama" in config["model_name"]:
        model = MoEsparseRouting(model, config)
        model.base_module.score = MoEsparseRoutingForClassification(base_classifier=model.base_module.score, 
                                                               config=config)
    else:
    ...
    if config["dataload_type"] == "single":
        if config["dataset_name"] in ["mrpc", "cola", "sst2", "qnli", "rte", "qqp"]:
            glue_type = "glue"
        elif config["dataset_name"] in ["boolq", "wic"]:
        ...
        train_dataset_dict, val_dataset_dict = load_mixed_datasets_with_expertlabels(config["model_name"],config["multiple_datasets"], config["tokenizer_path"], config["datamix_type"])
        ...
        print("-"*25,"After loading all the mixed datasets","-"*25)
        print("Train dataset shapes: ", train_dataset['input_ids'].shape, 
              train_dataset['attention_mask'].shape, 
              train_dataset['label'].shape, 
              train_dataset['expert_label'].shape)
        print("Validation dataset shapes: ", val_dataset['input_ids'].shape, 
              val_dataset['attention_mask'].shape, 
              val_dataset['label'].shape,
              val_dataset['expert_label'].shape)
...
def main():
    ...
    dataset_name = "mrpc" # if dataload_type single, options: mrpc, cola, sst2, qnli, rte, qqp, boolq, wic
    multiple_datasets= ["cola", "mrpc", "wic"] # combination of the datasets/experts both to be used
    ...
    print("Loaded Experts and their Keys",experts_dict.keys())
    ...
    config = {
        ...
        #hyperparameters
        "router_type" : "multi_layer", # {single_layer, multi_layer, attention}
