...
class MoEsparseRouting(nn.Module):
    ...
    def calculate_router_loss(self, gate_logits, expert_label):
        router_loss = F.cross_entropy(gate_logits, expert_label)
        # print("\nRouter Loss from function", router_loss)
        return router_loss

    def forward(self, input_ids=None, attention_mask=None, labels=None, expert_label=None):        
        
        X= input_ids
        X_temp = input_ids
        # print("expert_labels", expert_labels.shape)
        # print("\ntype of X_temp", X_temp.dtype)
        X= X.to(self.device)
